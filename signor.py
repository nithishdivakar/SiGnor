...
def get_post_details(root_path):
    '''
        yyyy-mm-dd-title-sep-dashes.tex
    '''
    f = open(os.path.join(root_path, '__pub.lish'), 'r')
...
def post_compile():
    ...
    for post_reco in _db.get_all('posts'):
        ...
        op_list.append(
            (post_reco['root_path'], post_reco['title'][:-4], slides))
...
def post_gen_one(post_reco):
    ...
    os.system('make')
    slides = [f for f in os.listdir(
        './') if f.endswith(_slide_ext) and f.startswith('slide')]
    os.chdir(owd)
    return slides
